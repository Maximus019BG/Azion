...
const getPlatformIcon = (platform: string) => {
  if (
    ...
    return <FaDesktop className="text-5xl text-blue-500" />;
  } else if (platform.toLowerCase().includes("mac")) {
    return <FaLaptop className="text-5xl text-gray-500" />;
  } else if (
    platform.toLowerCase().includes("android") ||
    platform.toLowerCase().includes("ios")
  ) {
    return <FaMobileAlt className="text-5xl text-green-500" />;
  }
  return null;
};
    ...
    <div className="p-6 w-full max-w-4xl mx-auto flex flex-col gap-6">
      ...
      <div className="flex flex-col gap-6">
        {sessions.map((session, index) => (
          <div
            key={index}
            className="bg-white p-6 rounded-xl shadow-lg flex flex-col w-full"
          >
            ...
            <button
              ...
              className="mt-auto px-4 py-2 bg-red-500 text-white rounded-lg shadow-md hover:bg-red-600 transition duration-200"
            >
              Remove Session
            </button>
          </div>
